...
export default function CampaignAssetOrderSheet({ campaignId }: CampaignAssetOrderSheetProps) {
  ...
  const isTikTokCampaign = data?.campaign.platforms.includes('TikTok') === true;
  ...
  const previewDriveImport = async () => {
    ...
    try {
      setDriveImportPreview(await marketingCampaignService.previewAssetOrderDriveImport(campaignId, folderUrl));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Không thể quét ảnh từ Google Drive.');
    } finally {
  ...
  const applyDriveImport = async () => {
    ...
    if (driveImportPreview.missingOrders.length > 0 && !window.confirm(
    ...
    try {
      const result = await marketingCampaignService.applyAssetOrderDriveImport(campaignId, folderUrl);
      await loadOrders();
      setDriveImportPreview(result);
      toast.success(`Đã nhập ${result.appliedOrders || result.mappedOrders} Order từ Drive; ${result.queuedSlots || 0} bài đã được đưa vào xử lý media.`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Không thể nhập ảnh Drive vào Order.');
    } finally {
  ...
  const awaitingAssetCount = (data?.slots || []).filter((slot) => slot.status === 'awaiting_assets').length;
      ...
      <section className="overflow-hidden rounded-2xl border-2 border-blue-300 bg-gradient-to-br from-blue-50 via-white to-emerald-50 shadow-sm">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-base font-extrabold text-slate-900">{isTikTokCampaign ? 'Nhập video TikTok từ Google Drive' : 'Nhập ảnh thiết kế từ Google Drive'}</h3>
                {awaitingAssetCount > 0 && (
                  <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-extrabold text-amber-800">
                    {awaitingAssetCount} bài đang chờ {isTikTokCampaign ? 'video' : 'ảnh'}
                  </span>
                )}
              </div>
              <p className="mt-1 text-xs leading-5 text-slate-600">
                Dán link thư mục để hệ thống tự ghép {isTikTokCampaign ? 'video' : 'ảnh'} vào từng bài theo số thứ tự, sau đó xem trước trước khi xác nhận.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 px-5 py-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {driveImportLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <FolderOpen className="h-4 w-4" />}
              Quét và xem trước
            </button>
          </div>
          <p className="rounded-lg bg-white/80 px-3 py-2 text-[11px] leading-5 text-blue-900">
            <strong>Quy tắc tên file:</strong> {isTikTokCampaign ? <><code>1.mp4</code> hoặc <code>1</code> nếu Drive nhận diện MIME video; video phải là MP4, MOV hoặc WebM.</> : <><code>1.jpg</code>, <code>2.png</code>; album dùng <code>3_1.jpg</code>, <code>3_2.jpg</code>.</>} Hệ thống không tự dồn file khi thiếu số.
          </p>
          {driveImportPreview && (
            <div className="rounded-xl border border-blue-200 bg-white p-3 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                ...
                >
                  {driveImportApplying && <Loader2 className="h-3.5 w-3.5 animate-spin" />} Xác nhận nhập {isTikTokCampaign ? 'video' : 'ảnh'}
                </button>
              </div>
              <div className="mt-2 grid gap-1 text-[11px] text-slate-600 sm:grid-cols-2">
                {driveImportPreview.mappings.slice(0, 8).map((mapping) => (
                  <div key={mapping.orderId} className="flex items-center justify-between gap-2 rounded-md bg-slate-50 px-2 py-1.5">
