...
export function AiCommentReplyManager({
        ...
        const mockLogs = [
          {
            ...
            aiResponse: "Chào bạn! iGen Marketing đã gửi bản PDF cẩm nang hướng dẫn chi tiết qua tin nhắn riêng cho bạn rồi nhé. Vui lòng kiểm tra hộp thư chứ!",
      ...
      if (res.ok && data.success && data.logs && data.logs.length > 0) {
        ...
        setPostDetails(prev => ({
          ...
          "mock_facebook_post_1": {
            message: "GIÁI PHÁP MARKETING TỰ ĐỘNG THẾ HỆ MỚI - Tiết kiệm 80% thời gian quản lý Fanpage, tự động chốt leads cực nhanh bằng AI!",
  ...
  const handleUploadLocalDoc = async (file: File) => {
    ...
    if (file.size > 10 * 1024 * 1025) {
      toast.error("Tập tin vượt quá dung lượng tối đa cho phép (10MB).");
      return;
    }
    ...
    try {
      ...
      reader.onerror = () => {
        toast.error("Không thể đọc tập tin.");
        setUploadingDoc(false);
      };
    } catch (err: any) {
      console.error(err);
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6 border-b border-gray-100 pb-4">
          <div className="text-left">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Phân tách quản lý các phản hồi tự động theo từng bài viết cụ thể trên {activePlatform === "facebook" ? "Fanpage Facebook" : "TikTok"} của bạn.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            ...
            <div className="flex bg-slate-100 p-0.5 rounded-xl border border-slate-200 text-[10px] font-extrabold shadow-xxs">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer ${activePlatform === "facebook"
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer ${activePlatform === "tiktok"
                ...
                <div className="p-4 space-y-4 text-left">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-1.5">
                      <input
                        type="text"
                        placeholder="Dán link Google Drive hoáº·c Doc/Sheet..."
                        value={driveLink}
                  ...
                  <div className="space-y-2">
                    <label className="block text-[10px] font-extrabold text-slate-500 uppercase tracking-wide flex items-center gap-1">
                      ...
                      Tải tập tin tài liệu trực tiếp
                    </label>
                    <label className={`flex flex-col items-center justify-center border border-dashed rounded-xl p-3 text-center cursor-pointer transition-all ${uploadingDoc
                      ...
                      <span className="text-[9px] font-bold text-slate-700">
                        {uploadingDoc ? "Đang xử lý..." : "Kéo thả hoặc click để chọn tập tin"}
                      </span>
                    </label>
                  </div>
                  ...
                  <div className="pt-3 border-t border-gray-200 text-left">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-gray-50 border border-gray-200 rounded-xl p-2 flex flex-col justify-between">
                        ...
                        <strong className={`mt-0.5 text-[10px] font-bold ${knowledgeHealth?.mode === "trained" ? "text-green-700" : "text-amber-700"
                          }`}>
                              ...
                              <div key={log._id} className="border border-gray-150 rounded-xl p-3 bg-gray-50/20 hover:bg-gray-50/50 transition-colors space-y-2.5">
                                <div className="flex justify-between items-center text-[10px]">
                                  <div className="flex items-center gap-1.5">
                                    <span className={`px-1.5 py-0.5 rounded-full font-bold uppercase ${(log.status === "sent" || log.status === "success")
                                ...
                                <div className="flex justify-between items-center text-[10px] pt-1">
                                  <div className="flex gap-2">
                                    <button
                                      ...
                                      className={`px-2 py-0.5 rounded-lg font-bold transition-all active:scale-95 cursor-pointer text-[9px] ${log.feedback === "good"
                                    ...
                                    <button
                                      ...
                                      className={`px-2 py-0.5 rounded-lg font-bold transition-all active:scale-95 cursor-pointer text-[9px] ${log.feedback === "needs_fix"
                                    ...
                                    <button
                                      ...
                                      className={`px-2 py-0.5 rounded-lg font-bold transition-all active:scale-95 cursor-pointer text-[9px] ${log.feedback === "bad"
