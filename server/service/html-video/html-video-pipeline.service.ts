...
export function compileHtmlVideoComposition(
  ...
  const html = `<main class="${deckClasses}">${visual.scenes.map((scene, index) => {
    ...
    const backgroundStage = hasBackgroundMedia
      ...
      : "";
    return `<section class="scene scene-${index + 1} layout-${scene.layout} ${compositionClass} ${surfaceClass} ${backgroundClass} ${motionClass} ${emphasisClass} ${accentClass} ${mediaClass}" data-scene-id="${escapeHtml(plannedScene?.id || scene.sceneId)}"${primaryUnitId ? ` data-unit-id="${escapeHtml(primaryUnitId)}" data-unit-ids="${escapeHtml(allUnitIds)}"` : ""}>${backgroundStage}<div class="scene-pattern"></div><div class="scene-band"></div><div class="scene-orb scene-orb-a" data-layout-allow-overflow="true"></div><div class="scene-orb scene-orb-b" data-layout-allow-overflow="true"></div><span class="scene-number">${String(index + 1).padStart(2, "0")}</span><div class="scene-frame"><div class="scene-copy">${text}</div>${media ? `<div class="scene-media">${media}</div>` : motif}</div></section>`;
