...
canvaRouter.get("/status",async(req:AuthenticatedRequest,res)=>{try{res.json({status:"success",data:await canvaService.status(req.user!.id)})}catch(e){res.status(400).json({status:"error",message:e instanceof Error?e.message:String(e)})}});
